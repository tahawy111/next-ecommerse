...
const Register = () => {
  ...
  const handleSubmit = async (e: IFormEvent) => {
    ...
    if (!userData.name) return show(`Name isn't exist`, "warn");
    if (!validEmail(userData.email)) return show(`Email isn't valid`, "warn");

    if (userData.password !== userData.confirmPassword)
      return show(`Passwords doesn't match.`, "warn");
    if (userData.password.length < 8 || userData.confirmPassword.length < 8)
      return show(`Passwords must be at least 8 chars.`, "warn");

    if (!containsNumbers(userData.password)) {
      return show("Passwords must be at least one numeric.", "warn");
    }
    if (!containsUppercase(userData.password)) {
      return show("Passwords must be at one uppercase.", "warn");
    }
