...
const DetailProduct = ({ product }: IProps) => {
    ...
    useEffect(() => {
        console.log(divImagesRef.current.naturalWidth)
    },[])
                ...
                <div className="grid">
                    ...
                    <div className={`w-[600px]`}>
                        ...
                        <div className={`flex mx-3 items-center justify-center`}>
                            <span className={`material-icons-outlined cursor-pointer select-none ${product.images.length < 7 && "pointer-events-none opacity-50"}`} onClick={() => setImgIndexTransform((prev) => prev + 1 > 0 ? 0 : prev + 1)}>
                                arrow_back_ios
                            </span>
                            ...
                            {/* <span className={`material-icons-outlined cursor-pointer select-none ${product.images.length < 7 && "pointer-events-none opacity-50"}`} onClick={() => setImgIndexTransform((prev) => prev - 1 < divImagesRef.current?.naturalWidth ? 0 : 1 )}> */}
                                {/* arrow_forward_ios */}
                            {/* </span> */}
                        </div>
                    </div>
